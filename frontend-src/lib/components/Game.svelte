<script lang="ts">
  ...
  function buildStaticBgTexture(): Texture {
    ...
    const topBloom = s.createRadialGradient(
      TW / 2,
      -30,
      10,
      TW / 2,
    ...
    const vignette = s.createRadialGradient(
      TW / 2,
      TH / 2,
      TH * 0.18,
      TW / 2,
  ...
  function handleAudio(state: RS) {
    ...
    const nearNow =
      inGap && (isHost ? state.puck[1] > myGoalY : state.puck[1] < myGoalY);
  ...
  function drawGoals(g: Graphics, wf: number) {
    ...
    g.moveTo(GX, 0)
      .lineTo(GX, 16)
      .stroke({ width: 2.8, color: 0x34c8a1, alpha: 0.74 + wg });
    g.moveTo(GX + GOAL_W, 0)
      .lineTo(GX + GOAL_W, 16)
      .stroke({ width: 2.8, color: 0x34c8a1, alpha: 0.74 + wg });
    g.rect(GX + 4, 2, GOAL_W - 8, 2.6).fill({
      color: 0xc6f8e4,
      alpha: 0.54 + wg,
    });
    for (let x = GX + 6; x <= GX + GOAL_W - 6; x += 10) {
      g.moveTo(x, 5)
        .lineTo(x + 5, 15)
        .stroke({ width: 0.9, color: 0x85e8c9, alpha: 0.18 + wg });
    }

    g.rect(GX, TH - 16, GOAL_W, 16).fill({
      color: 0x5e93de,
      alpha: 0.16 + wg + pulse,
    });
    g.moveTo(GX, TH)
      .lineTo(GX, TH - 16)
      .stroke({ width: 2.8, color: 0x5e93de, alpha: 0.74 + wg });
    g.moveTo(GX + GOAL_W, TH)
      .lineTo(GX + GOAL_W, TH - 16)
      .stroke({ width: 2.8, color: 0x5e93de, alpha: 0.74 + wg });
    g.rect(GX + 4, TH - 4.6, GOAL_W - 8, 2.6).fill({
      color: 0xd3e6ff,
      alpha: 0.54 + wg,
    });
    for (let x = GX + 6; x <= GX + GOAL_W - 6; x += 10) {
      g.moveTo(x, TH - 15)
        .lineTo(x + 5, TH - 5)
        .stroke({ width: 0.9, color: 0xa7cfff, alpha: 0.18 + wg });
    }
  }
  ...
  function drawMidline(g: Graphics, wf: number) {
    ...
    for (let x = 0; x < TW; x += 22) {
      ...
      g.rect(x, TH / 2 - lw / 2, segW, lw).fill({
        color: alt ? 0xf2dfb1 : 0x7eb5ef,
        alpha,
      });
    }
  }
  ...
  function drawPuckFx(
    ...
    g.circle(px, py, ringR).stroke({
      width: 1.6 + speedGain * 0.8,
      color: 0xf2f6fb,
      alpha: 0.28 + speedGain * 0.2,
    });
    g.circle(px, py, PR * 0.48 + pulse * 1.2).fill({
      color: 0xf8fbff,
      alpha: 0.14 + speedGain * 0.12,
    });
    g.circle(px, py, PR * 0.3 + pulse * 0.7).stroke({
      width: 1.1,
      color: 0xe6edf5,
      alpha: 0.4 + speedGain * 0.1,
    });

    const orbit = PR * 0.64;
    const a = now * 0.012;
    g.circle(px + Math.cos(a) * orbit, py + Math.sin(a) * orbit, 1.8).fill({
      color: 0x9cc9f2,
      alpha: 0.36 + speedGain * 0.2,
    });
    g.circle(
      px + Math.cos(a + Math.PI) * orbit,
      py + Math.sin(a + Math.PI) * orbit,
      1.6,
  ...
  function drawPuckDecal(g: Graphics, px: number, py: number, now: number) {
    ...
    g.circle(px + dx, py + dy, PR * 0.12).fill({
      color: 0x60a5fa,
      alpha: 0.88,
    });
    g.circle(px - dx, py - dy, PR * 0.12).fill({
    ...
    g.circle(px, py, PR * 0.36).stroke({
      width: 1.2,
      color: 0xffffff,
      alpha: 0.72,
    });
    g.moveTo(px - PR * 0.2, py)
      .lineTo(px + PR * 0.2, py)
      .stroke({ width: 1.1, color: 0x111827, alpha: 0.8 });
    g.moveTo(px, py - PR * 0.2)
      .lineTo(px, py + PR * 0.2)
      .stroke({ width: 1.1, color: 0x111827, alpha: 0.8 });
  }
  ...
  function drawActorFallback(
    ...
    g.circle(blue[0], blue[1], PAR * 0.9).fill({
      color: 0x60a5fa,
      alpha: 0.36,
    });
    g.circle(blue[0], blue[1], PAR * 0.9).stroke({
      width: 2.2,
      color: 0xdbeafe,
      alpha: 0.9,
    });

    g.circle(green[0], green[1], PAR * 0.9).fill({
      color: 0x34d399,
      alpha: 0.36,
    });
    g.circle(green[0], green[1], PAR * 0.9).stroke({
      width: 2.2,
      color: 0xd1fae5,
      alpha: 0.9,
    });

    g.circle(puck[0], puck[1], PR * 0.86).fill({
      color: 0xf59e0b,
      alpha: 0.52 + speedGain * 0.18,
    });
    g.circle(puck[0], puck[1], PR * 0.86).stroke({
      width: 2,
      color: 0xfff7c2,
      alpha: 0.95,
    });
  ...
  function updateScores() {
    ...
    if (rs.score_flash[opIdx] > 0.01) {
      scoreGlowGfx
        .circle(54, pxH / 2 - 44, 52 + rs.score_flash[opIdx] * 24)
        .fill({
    ...
    if (rs.score_flash[myIdx] > 0.01) {
      scoreGlowGfx
        .circle(54, pxH / 2 + 52, 52 + rs.score_flash[myIdx] * 24)
        .fill({
    ...
    opScoreBg.clear();
    opScoreBg
      .roundRect(10, pxH / 2 - 8 - opSz - 12, opM.width + 34, opSz + 20, 18)
      .fill({ color: 0x02060d, alpha: 0.42 });
    opScoreBg
      .roundRect(10, pxH / 2 - 8 - opSz - 12, opM.width + 34, opSz + 20, 18)
      .stroke({ width: 1.5, color: isHost ? 0x4ad9aa : 0x76c8ff, alpha: 0.34 });
    opScoreBg
      .rect(
        20,
        pxH / 2 - 8 - opSz + opSz + 1,
        Math.max(16, opM.width + 14),
        2.4,
      )
      .fill({ color: isHost ? 0x4ad9aa : 0x76c8ff, alpha: 0.45 });
    ...
    myScoreBg.clear();
    myScoreBg
      .roundRect(10, pxH / 2, myM.width + 34, mySz + 20, 18)
      .fill({ color: 0x02060d, alpha: 0.42 });
    myScoreBg
      .roundRect(10, pxH / 2, myM.width + 34, mySz + 20, 18)
      .stroke({ width: 1.5, color: isHost ? 0x76c8ff : 0x4ad9aa, alpha: 0.34 });
    myScoreBg
      .rect(20, pxH / 2 + mySz + 7, Math.max(16, myM.width + 14), 2.4)
      .fill({ color: isHost ? 0x76c8ff : 0x4ad9aa, alpha: 0.45 });
  }
  ...
  function updateCountdown() {
    ...
    countdownPanel
      .roundRect(pxW / 2 - pw / 2, ly - ph / 2, pw, ph, R * 0.35)
      .fill({ color: 0x050d1b, alpha: 0.8 });
    countdownPanel
      .roundRect(pxW / 2 - pw / 2, ly - ph / 2, pw, ph, R * 0.35)
      .stroke({ width: 1.2, color: 0x27405f, alpha: 0.6 });
    countdownPanel
      .rect(pxW / 2 - pw / 2, ly - 2, pw, 4)
      .fill({ color: 0x11233d, alpha: 0.9 });
  ...
  function update() {
    ...
    const myPos =
      localPaddlePos ?? (isHost ? rs.host_paddle : rs.client_paddle);
    ...
    const needsActorFallback =
      ...
      (puckBodySpr.texture.width <= 0 ||
        paddleBlueSpr.texture.width <= 0 ||
        paddleGreenSpr.texture.width <= 0);
    ...
    if (rs.goal_flash > 0) {
      ...
      goalFlashGfx
        .rect(0, 0, TW, TH)
        .fill({ color: 0xffffff, alpha: rs.goal_flash * 0.25 });
    } else {
  ...
  onMount(async () => {
    ...
    app.canvas.addEventListener("pointerdown", onPointerMove, {
      passive: false,
    });
...
{#if gameOver}
  <div
    class="fixed inset-0 flex flex-col items-center justify-center z-10 bg-black/80 backdrop-blur-md"
  >
    <div
      class="flex flex-col items-center gap-6 p-12 rounded-[2.5rem] bg-gradient-to-br from-neutral-900/95 to-neutral-800/90 border border-neutral-600/50 shadow-[0_0_60px_rgba(0,0,0,0.6)] animate-in fade-in zoom-in duration-300"
    >
      <div class="text-7xl {iWon ? 'animate-bounce' : 'animate-pulse'}">
        {iWon ? "🏆" : "😔"}
      </div>
      <h2
        class="text-5xl font-black {iWon
          ? 'text-yellow-400 drop-shadow-[0_0_24px_rgba(250,204,21,0.6)]'
          : 'text-neutral-400'}"
      >
        {iWon ? "VICTORY!" : "DEFEAT"}
      </h2>
      <div
        class="flex items-center gap-4 text-4xl font-black text-white tracking-widest bg-neutral-800/50 px-8 py-4 rounded-2xl border border-neutral-600/30"
      >
        <span
          class={rs.score[0] > rs.score[1]
            ? "text-yellow-400"
            : "text-neutral-400"}>{rs.score[0]}</span
        >
        <span class="text-neutral-600">-</span>
        <span
          class={rs.score[1] > rs.score[0]
            ? "text-yellow-400"
            : "text-neutral-400"}>{rs.score[1]}</span
        >
      </div>
      <div class="flex gap-3 mt-4 w-full">
        {#if isSinglePlayer}
          <button
            class="flex-1 py-4 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-2xl text-lg font-bold hover:from-emerald-500 hover:to-emerald-400 active:scale-95 uppercase tracking-widest shadow-[0_0_24px_rgba(16,185,129,0.4)] border border-emerald-400/30 transition-all"
            onclick={rematch}>🔄 Play Again</button
          >
        {/if}
        <button
          class="flex-1 py-4 bg-gradient-to-r from-neutral-700 to-neutral-600 text-white rounded-2xl text-lg font-bold hover:from-neutral-600 hover:to-neutral-500 active:scale-95 uppercase tracking-widest shadow-lg border border-neutral-500/30 transition-all"
          onclick={() => onBack?.()}>🏠 Menu</button
        >
      </div>
    </div>
  </div>
{:else}
  ...
  <button
    ...
    onclick={() => (muted = !muted)}>{muted ? "🔇" : "🔊"}</button
  >
{/if}
